...
class SkuModel extends BaseModel
{
    ...
    public function addSku($pid,$arr){
        $json=$this->setJson($arr);
        $res=self::create([
            'json' => $json,
            'goods_id' => $pid
        ]);
    ...
    public function editSku($pid,$arr){
        $json=$this->setJson($arr);
        $gsku = self::where('goods_id',$pid)->find();
        if ($gsku) {
            self::where('goods_id',$pid)->update(['json'=>$json]);
        } else {
            self::create([
                'json' => $json,
                'goods_id' => $pid
    ...
    {
        $sarr = $this->set_kv_arr($arr);  //sku_tree
    ...
    public function set_kv_arr($arr)
    {
        $obj = [];
        foreach ($arr as $k => $v) {
            foreach ($v as $kk => $vv) {
